 ...
 */
function prepareMessage(ctx: CTX, msg: string, ...data: any[]) {
...
const logger = winston.createLogger({
  transports: [
    new winston.transports.File({ filename: '/bot-log.log', level: 'debug' }),
  ],
  format: combine(timestamp(), format.splat(), format.simple(), logFormat),
});

export default {
  debug: (ctx: CTX, msg: string, ...data: any[]) =>
    logger.debug(prepareMessage(ctx, msg, ...data)),
  error: (ctx: CTX, msg: string, ...data: any[]) =>
